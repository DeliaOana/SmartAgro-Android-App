...
*/
    ...
    public void messageArrived(String topic, MqttMessage message)
        ...
        notification.setContentTitle("SmartAgro");
        ...
        if(ch[0]=='A'){
            ...
            notification.setStyle(new NotificationCompat.BigTextStyle().bigText(notification_message));
            notificationManager.notify(uniqueID, notification.build());
        }
        /*
        if(temp_sol_int<15 || temp_sol_int>25 || temp_aer_int<15 || temp_aer_int>30 || umid_aer_int<50 || umid_sol_int<50 || umid_sol_int>70 || pres_atm_int<100 || pres_atm_int>110){
            String mesaj_notificare = "Atentie! ";
            if(temp_sol_int<15){
                mesaj_notificare = mesaj_notificare + "Temperatura sol scazuta. ";
            }
            if(temp_sol_int>25){
                mesaj_notificare = mesaj_notificare + "Temperatura sol ridicata. ";
            }
            if(temp_aer_int<15){
                mesaj_notificare = mesaj_notificare + "Temperatura aer scazuta. ";
            }
            if(temp_aer_int>30){
                mesaj_notificare = mesaj_notificare + "Temperatura aer ridicata. ";
            }
            if(umid_aer_int<50){
                mesaj_notificare = mesaj_notificare + "Umiditate aer scazuta. ";
            }
            if(umid_sol_int<50){
                mesaj_notificare = mesaj_notificare + "Umiditate sol scazuta. ";
            }
            if(umid_sol_int>70){
                mesaj_notificare = mesaj_notificare + "Umiditate sol crescuta. ";
            }
            if(pres_atm_int<100){
                mesaj_notificare = mesaj_notificare + "Presiune atmosferica scazuta. ";
            }
            if(pres_atm_int>110){
                mesaj_notificare = mesaj_notificare + "Presiune atmosferica crescuta. ";
            }
            notification.setContentIntent(pendingIntent);
            ...
            switch (paramString[0]){
                ...
                case "connect":
                    ...
                    try {
                        ...
                        options.setUserName("smartagro");
                        options.setPassword("MckETMys67AYkahU".toCharArray());
